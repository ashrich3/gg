...
character_guidelines = {
    "blair": {
        ...
        "signature_moves": [
            ...
            "Speaks in queen bee declarations"
        ]
    },
    "serena": {
...
dialogue_style = {
    "tempo": "Fast-paced with overlapping emotional beats",
    "banter": "Characters must use banter to hide deeper truths",
    "subtext": "Every conflict should carry hidden longing, insecurity, or rivalry",
...
character_profiles = {
    ...
    "blair": {
        "name": "Blair Waldorf",
        "traits": ["intelligent", "manipulative", "insecure", "romantic"],
        "relationships": {
            "serena": "best friend",
            "chuck": "true love",
...
def home():
    ...
    return jsonify({
        ...
        "endpoints": {
            "episodes": "GET /recaps/season/<season>/episode/<episode>",
            "all_episodes": "GET /recaps", 
            ...
            "single_scene_guidance": "GET /scene-guidance/<name>"
        },
...
def get_canon_rules():
    """
    Get canon rules for the Gossip Girl universe
    """
    return jsonify(canon_rules)
...
@app.route('/character-guidelines/<name>', methods=['GET'])
def get_character_guidelines(name):
    """
    Get character guidelines for writing
    """
    key = name.lower()
    if key in character_guidelines:
        return jsonify(character_guidelines[key])
    else:
        return jsonify({"error": f"No guidelines found for {name}"}), 404

@app.route('/dialogue-style', methods=['GET'])
...
def get_single_scene_guidance(name):
    """
    Returns trimmed guidance for a single character including tone, traits, writing tips, and canon rules.
    """
    ...
    return jsonify({
        "name": name,
        "tone": char_data.get("dialogueStyle", {}).get("tone", ""),
        "writingTips": guidelines.get("rules", [])[:5],  # Limit to top 5 tips
        "traits": char_data.get("traits", [])[:5],       # Limit to top 5 traits
        "relationships": char_data.get("relationships", {}),
        "emotionalTriggers": char_data.get("emotionalTriggers", [])[:3],
        "forbiddenCanon": forbidden
    })
